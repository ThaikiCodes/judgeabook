...
#################### PACKAGE ACTIONS ###################
reinstall_package:
	@pip uninstall -y judgeabook || :
	@pip install -e .

run_preprocess:
	python -c 'from judgeabook.interface.main import preprocess; preprocess()'

run_train:
	python -c 'from judgeabook.interface.main import train; train()'

run_pred:
	python -c 'from judgeabook.interface.main import pred; pred()'

run_evaluate:
	python -c 'from judgeabook.interface.main import evaluate; evaluate()'
